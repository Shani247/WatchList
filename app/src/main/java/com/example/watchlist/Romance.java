...
public class Romance extends BaseActivity {
    TextView listR, watched;
    SharedPreferences sp, spWatched;
    String[] tempArr;
    @Override
    ...
    private void initViews() {
        ...
        listR.setText(spannable);
        listR.setMovementMethod(LinkMovementMethod.getInstance()); // חובה כדי שהלחיצות יעבדו

        watched = findViewById(R.id.watched);
        spWatched = getSharedPreferences("movies_status", 0);
        String contectW = "";
        for (int i = 0; i < tempArr.length; i++){
    ...
    private void showMovieDialog(String movieName) {
        ...
        btnPhoto.setOnClickListener(v -> {
            Intent intent = new Intent(Romance.this, posters.class);
